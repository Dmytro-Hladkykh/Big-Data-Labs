...
class TopAirlinesDepartureDelay(MRJob):

    def mapper(self, _, line):
        fields = line.split(',')
        if len(fields) >= 12:
            airline_code = fields[4]
            departure_delay_str = fields[11]
            if departure_delay_str:
                try:
                    departure_delay = float(departure_delay_str)
